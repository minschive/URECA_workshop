...
<div class="container mt-3">
    ...
    <script>
        ...
        function makeListHtml(list){
            ...
            list.forEach( el => {
                let userSeq = el.userSeq;
                let userName = el.userName;
                let userEmail = el.userEmail;
                let userPassword = el.userPassword;
                let userProfileImage = el.userProfileImage;
                let userRegisterDate = el.userRegisterDate;

                listHTML += `<tr style="cursor:pointer" data-userSeq=\${userSeq}>
                            <td>\${userSeq}</td>
                            <td>\${userName}</td>
                            <td>\${userEmail}</td>
                            <td>\${userPassword}</td>
                            <td>\${userProfileImage}</td>
                            <td>\${userRegisterDate}</td>
                        </tr>`;
            });

            document.querySelector("#cusTbody").innerHTML = listHTML;

            document.querySelectorAll("#cusTbody tr").forEach( el => {
                el.onclick = function(){
                    let user_seq = this.getAttribute("data-userSeq");
                    detailCus(userSeq);
                }
            } );
        }

        async function detailCus(userSeq) {
            alert(userSeq);
        }
    </script>
</body>
